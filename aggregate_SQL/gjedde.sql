DROP TABLE IF EXISTS fremmedfisk.gjedde;
CREATE TABLE fremmedfisk.gjedde AS
SELECT *,
    CASE
...
SELECT a.*, nv.verneform, nt.naturtype_tekst AS naturtype, nt."bmvVerdi_tekst" AS naturtype_verdi,
	...
	CASE WHEN n_intro_avg_scenario_0_med_egenspredning = 0 THEN 0.0 ELSE
    (n_intro_scenario_0_uten_egenspredning - n_intro_scenario_2_uten_egenspredning) / n_intro_avg_scenario_0_med_egenspredning END AS effekt_scenario_2_uten_egenspredning_prosent,
	CASE WHEN n_intro_avg_scenario_0_med_egenspredning = 0 THEN 0.0 ELSE
    (n_intro_scenario_0_uten_egenspredning - n_intro_scenario_1_uten_egenspredning) / n_intro_avg_scenario_0_med_egenspredning END AS effekt_scenario_1_uten_egenspredning_prosent

 FROM (SELECT x.*, m.municipality FROM (
    SELECT
        l.geom, l.waterbody, l.area_km2, t.*, CAST('Gjedde' AS text) AS species, ft."forekomst_oerret", ft."forekomst_harr", ft."forekomst_gjedde", ft."forekomst_oerkyte", ft."forekomst_solabbor" 
		FROM
        (SELECT
            *
        FROM
            ...
            FROM
                (SELECT "waterBodyID", CAST(n_intro AS smallint), p_intro FROM fremmedfisk.sim_esox_lucius_scenario_2_500_simu_600_deg UNION ALL
                SELECT "waterBodyID", CAST(n_intro AS smallint), p_intro FROM fremmedfisk.sim_esox_lucius_scenario_2_500_simu_700_deg UNION ALL
                SELECT "waterBodyID", CAST(n_intro AS smallint), p_intro FROM fremmedfisk.sim_esox_lucius_scenario_2_500_simu_800_deg) AS emt_a GROUP BY "waterBodyID") AS s2me LEFT JOIN
			(SELECT "waterBodyID",
            ...
            FROM
                (SELECT "waterBodyID", CAST(n_intro AS smallint), p_intro FROM fremmedfisk.sim_esox_lucius_scenario_1_500_simu_600_deg UNION ALL
                SELECT "waterBodyID", CAST(n_intro AS smallint), p_intro FROM fremmedfisk.sim_esox_lucius_scenario_1_500_simu_700_deg UNION ALL
                SELECT "waterBodyID", CAST(n_intro AS smallint), p_intro FROM fremmedfisk.sim_esox_lucius_scenario_1_500_simu_800_deg) AS emt_a GROUP BY "waterBodyID") AS emt USING ("waterBodyID") LEFT JOIN
            (SELECT "waterBodyID",
            ...
            FROM
                (SELECT "waterBodyID", CAST(n_intro AS smallint), p_intro FROM fremmedfisk.sim_esox_lucius_scenario_0_500_simu_600_deg UNION ALL
                SELECT "waterBodyID", CAST(n_intro AS smallint), p_intro FROM fremmedfisk.sim_esox_lucius_scenario_0_500_simu_700_deg UNION ALL
                SELECT "waterBodyID", CAST(n_intro AS smallint), p_intro FROM fremmedfisk.sim_esox_lucius_scenario_0_500_simu_800_deg) AS eut_a
                GROUP BY "waterBodyID") AS eut NATURAL LEFT JOIN
            (SELECT "waterBodyID", CAST(n_intro AS smallint) AS n_intro_scenario_2_uten_egenspredning, p_intro AS p_intro_scenario_2_uten_egenspredning FROM fremmedfisk.sim_esox_lucius_scenario_2_500_simu_no_second) AS s2ue USING ("waterBodyID") LEFT JOIN
            (SELECT "waterBodyID", CAST(n_intro AS smallint) AS n_intro_scenario_1_uten_egenspredning, p_intro AS p_intro_scenario_1_uten_egenspredning FROM fremmedfisk.sim_esox_lucius_scenario_1_500_simu_no_second) AS imt USING ("waterBodyID") LEFT JOIN
            (SELECT "waterBodyID", CAST(n_intro AS smallint) AS n_intro_scenario_0_uten_egenspredning, p_intro AS p_intro_scenario_0_uten_egenspredning FROM fremmedfisk.sim_esox_lucius_scenario_0_500_simu_no_second) AS iut USING ("waterBodyID")) AS t NATURAL LEFT JOIN
